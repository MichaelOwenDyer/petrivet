...
## 0. The inversion, stated plainly

The rest of this corpus has a stone and a song. The stone is the certificate-and-checker — a machine-checkable proof for every verdict, re-validated by a small external checker that is the entire trusted base ([the-checkable-frontier.md](the-checkable-frontier.md); [core-principles.md](core-principles.md) Principle 4). The song, in [soundness-as-a-free-variable.md](soundness-as-a-free-variable.md), is that soundness is independent of the selection policy. Both are true. Neither is falsifiable in the empirical sense, because both are theorems: the soundness firewall is a one-line corollary of certifying algorithms composed with algorithm selection, and the checker's correctness is a proof obligation, not a measurement.

A thesis needs a claim that the world could refute. This essay is about that claim, and it is deliberately the *empirical* one:
...
The figure of merit is **`f_struct`**: the fraction of queries the structural tier decides, reported against two denominators, with whole families held out. Everything else in the corpus is in service of making this one number trustworthy. The certificate is what makes a decided query *believable*; the firewall is what makes the routing *safe*; but the thesis — the part that could be wrong — is that `f_struct` is large, characterizable, cheap, and predictable on real models.

This is the ratified inversion of the documents' earlier emphasis (BACKLOG.md, *The organizing thesis*): the certificate-and-checker is the contribution; **the structural-coverage claim is the headline, and the headline is falsifiable.**
...
First, **every decided query carries a checked certificate** ([the-checkable-frontier.md](the-checkable-frontier.md); BACKLOG.md C1/C2/G6). A structural verdict is not a bare boolean to be trusted; it is a firing word, a Farkas P-invariant (one dot product to check), an exhibited siphon/trap cover (linear to check even though enumerating all siphons is hard), a place-invariant bound. The certifying fraction `f` — the share of accepted verdicts that carry a checked certificate — is the companion figure of merit, and the in-band check (BACKLOG.md C4) means a query counted in `f_struct` has had its proof re-validated against the *original* net before it was counted. Coverage you cannot check is coverage you cannot report.

Second, **the firewall holds** ([soundness-as-a-free-variable.md](soundness-as-a-free-variable.md)): the routing that sends a query to the structural tier rather than to search cannot, by the policy-independence theorem, turn a wrong answer into an accepted one. This is what lets `f_struct` be a coverage number rather than a coverage-and-correctness gamble: the firewall decouples *which tier fired* from *whether the verdict is true*. The two-denominator coverage table and the certifying-fraction table (G4/G6) are therefore measuring orthogonal things — reach and trustworthiness — and both must be reported.
