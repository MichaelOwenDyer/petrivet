...
> Status: vision essay. This is exploratory writing (Daniel Dyer, with Claude), built on Michael's tool. It argues that the project's *signature technical contribution* is the certificate-and-checker, names the format, identifies the one load-bearing design decision, and draws the per-property × polarity hardness map that is the heart of the contribution. The core Petri-net theory is Michael's; the framing and the map are proposed here. Companion to [`soundness-as-a-free-variable.md`](soundness-as-a-free-variable.md), which proves that this apparatus makes a learned scheduler safe, and to [`the-coverage-claim.md`](the-coverage-claim.md), which measures how much of the corpus it reaches.

---

## 1. The missing artifact

The SAT, SMT, and ILP communities settled a question that Petri-net model checking has left open: *what does a model checker hand you when it says "no"?* In SAT the answer is a **DRAT** proof — a sequence of clause additions and deletions that a small, separately written checker replays against the original CNF, deciding UNSAT without trusting the solver that produced it. **LRAT** and **GRAT** add the hints that make checking linear and the checker formally verifiable; the SAT community trusts only `gratchk`, a few hundred lines, while the proof *generator* `gratgen` stays unverified and fast (Lammich, *CADE* 2017). **VeriPB** extends the same discipline to pseudo-Boolean and cutting-plane reasoning, where a single resolution refutation no longer suffices. SMT solvers emit proof terms that an external kernel re-derives. The shared invariant across all of them: *the prover is untrusted; a small checker re-establishes the claim against the original problem; the proof is a serialized artifact that crosses tool boundaries.*
...
- **Negative reachability and coverability (rational)** carry a *Farkas dual* / P-semiflow `y`: a place-weighting with `y·C = 0` and `y·(target − M₀) ≠ 0`. This is the conservation law that *explains* the impossibility — the discarded dual of [`core-principles.md`](core-principles.md) §2.2.a. Checking it is one exact dot product. The witness is computed today and thrown away at `reachability.rs:177` and `coverability.rs:124`; emitting it converts every LP-refuted "no" into a near-linear-checkable verdict symmetric to the positive ones. **The exactness caveat is load-bearing:** the dual must be checked over ℚ, because a *floating* dual is not a proof and a spurious floating "infeasible" is a silent false `Unreachable` (backlog B1a) — the one negative-path hazard the firewall does not catch, since there is no positive object to re-check.
...
The discipline the table enforces is the [claim-honesty method](core-principles.md) §4.3 applied to *certificates*: a cell is either near-linear, or polynomial-with-an-exponential-generator, or on the wall — and each label states which, with its justification. The map's value is that it draws the line precisely.
...
The relationship to the rest of the corpus closes cleanly. [`soundness-as-a-free-variable.md`](soundness-as-a-free-variable.md) shows that once the checker exists, a learned scheduler is *safe* — soundness is policy-independent, the learner sits outside the trusted base. This essay is the prerequisite: there is no policy-independence theorem without the checker the theorem is about, and the §3 in-band check is the very line of code the soundness proof relies on. [`the-factorization-residual.md`](the-factorization-residual.md) measures what *cannot* be coarse-grained — the Φ_PN residual; the present essay measures what *can* be checked. And [`latent-architecture.md`](latent-architecture.md) reads the evidence/decision structures from which all of this is built.
