...
pub enum PnmlConversionError {
    ...
    /// of petrivet's token representation (E4). Silently clamping it to
    /// `u32::MAX` would import a *structurally different* net than the input —
    /// wrong-net-in, confident-answer-out (A7) — so this is a hard error rather
    ...
    /// An arc carries a non-unit `<inscription>` weight (E7). petrivet's native
    /// model represents only unit-weight arcs; silently linearising a weight-`k`
    /// arc to weight-1 would import a structurally different net than the input
    /// (A7), contrary to the converter's own discipline. Rejected rather than
...
fn convert_pt_net(
    ...
    for &pnml_arc in &flat.arcs {
        ...
        // A7/E7: petrivet's native model represents only unit-weight arcs. An
    ...
    //
    // A7/E4: a token count above `u32::MAX` is a hard error, not a silent
...
<pnml xmlns="http://www.pnml.org/version-2009/grammar/pnml">
  <net id="n" type="http://www.pnml.org/version-2009/grammar/ptnet">
    ...
    /// A7/E7 regression: a non-unit-weight P/T arc must be REJECTED, not
    ...
    /// A7/E4 regression: an initial marking above `u32::MAX` must be REJECTED,
