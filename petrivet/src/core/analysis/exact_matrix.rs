//! M5 · B0 (matrix half) — exact linear algebra over [`Rational`], the layer the
//! checkable algebraic certificates (Farkas duals, P/T-semiflows, exact rank)
//! are built on.
//!
...
//! theory is unsound (§1, §4.1): rank is *discontinuous* in the entries, and a
...
//!   that closes the silent false-`Unreachable` hole (B1a).
//!
//! # Exactness, and the Bareiss note (claim-honest)
//!
...
//! soundness property M5 requires. The fraction-free **Bareiss** schedule (which
//! keeps intermediate *integer* determinants polynomially bounded by the Hadamard
//! inequality) is the cost *optimization* the design names; it is a drop-in
//! refinement of the elimination order that does not change the results or the
//! interface. It is recorded as a follow-on: M5's gate is *exact*, which the
//! `Rational` reduction delivers; making it *fast at scale* is the Bareiss
//! schedule, deferred until a measured cost need (the same lean discipline as the
//! M4 `i128`-not-bignum call). See `foundational-design.md` §4.3.

// `dead_code`: several matrix primitives (`from_rows`, `from_ints`, `rank`,
// `mul_vector`, …) are foundation consumed by M6+ (the cluster/rank theorem) and
// the exact deciders; they are built ahead of those consumers (doctrine #7 — the
// blueprint is real) and exercised by this module's `[PROP]` tests now. Remove
// this allow as each gains a non-test consumer.
#![allow(dead_code)]
...
impl Matrix {
    ...
    /// *certificate* rather than the absence of an f64 solution (B1a): the verdict
...
/// `C·x = m' − m₀` over ℚ (sign-free). This is the B1a guard: it re-derives the
/// infeasibility that the `f64` LP only *suggests*, and supplies the exact
/// witness when it confirms it.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum MarkingEquationExact {
    ...
    /// `f64` infeasibility at a degenerate vertex lands here — the B1a hole, now
    /// caught.)
    Feasible,
...
/// place sub-invariant. This is the covering analogue of the B1a guard
/// ([`MarkingEquationExact`]): a sound *sufficient* exact certificate of
/// uncoverability, used to demote the `f64` covering LP from a verdict source to a
/// filter (M3).
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// # Method (exact, no `f64`)
///
/// The P-semiflows `{y : yᵀ·C = 0}` are the left kernel of `C` (M5). Every P-semiflow
...
/// exact. Full exact covering-LP duality is a B-epic follow-on; soundness here does
/// not depend on completeness.)
#[must_use]
...
mod tests {
    ...
    /// The B1a guard: `marking_equation_exact` reports `Infeasible` with an exact
    /// separating invariant for a token-non-conserving target on a conservative
    /// net, and `Feasible` for a reachable target — so a genuinely-reachable
    /// target is never reported infeasible (the falsifier the M0 test pins, now
    /// closed exactly).
    #[test]
    fn marking_equation_exact_guard_separates_infeasible_from_feasible() {
        ...
        let m0 = net.mapping.idx_marking([(p0, 1)].into());

        // Infeasible target: a different token sum.
        let bad = net.mapping.idx_marking([(p0, 2)].into());
        ...
        // NOT infeasible (this is the B1a falsifier: a reachable target is never
        // reported infeasible).
        let good = net.mapping.idx_marking([(p1, 1)].into());
        assert_eq!(
            marking_equation_exact(&net.dense_net, &m0, &good),
            MarkingEquationExact::Feasible,
            "B1a: a reachable target's equality system must be exactly feasible"
        );
    }
