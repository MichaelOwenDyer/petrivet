//! Cluster-quotient keystone (B2): the free-choice cluster partition and the
//! Rank-Theorem relation `rank(C) == c − 1`.
//!
...
//! registers **no** decider. The S/T-component and siphon deciders that close the
//! gap to the full theorem are deferred (backlog B3–B6) and consume this keystone.
//!
...
// The cluster partition and the Rank-Theorem relation are the keystone consumed
// by the deferred S/T-component deciders (backlog B3–B6, M8); they are built ahead
// of those consumers (doctrine #7 — the blueprint is real) and exercised by this
// module's tests now. Remove this allow as each gains a non-test consumer.
#![allow(dead_code)]
