//! An exact rational scalar with a documented overflow policy.
//!
//! A Petri net's linear theory is defined over ℚ; checkable algebraic
//! certificates (the Farkas dual, P/T-semiflows, exact rank) cannot be
//! produced over `f64`: a floating feasibility result near a constraint
//! boundary can be wrong, and rank is *discontinuous* in the entries, so
//! rounding is unsound. This module supplies the scalar the exact matrix
//! layer ([`exact_matrix`](super::exact_matrix)) is built on.
//!
//! # The representation decision
//!
//! `Rational` is a normalized `i128 / i128`. Three candidate representations
...
//!   ([`RationalOverflow`]), **never a silent wrap**. Integer wrap is treated
//!   exactly like floating-point rounding — a wrong magnitude is a wrong
//!   certificate, so neither is permitted on a verdict path. `i128` carries
//!   the elimination intermediates for realistic net sizes; when it does not,
//!   the operation *fails loudly* and the caller escalates or abstains rather
...
//!
//! This decision affects the cost of every downstream exact computation; the
//! trade-offs are recorded here so the choice can be re-derived or revisited.
//!
...
//! same bits*, because the only constructor normalizes. The
//! "value-equality independent of representation" test below rests on this.

// `dead_code`: the exact matrix layer consumes the core `Rational` surface
// (construction, the checked arithmetic, ordering), but a few convenience members
// (`From<i32>`, `denominator`, `Display`) are exercised only by tests or held for
// planned exact deciders and certificate rendering. They are kept as the type's
// complete, documented surface rather than trimmed and re-added; the narrow
// allow says so without masking a genuinely-unused *core* method.
#![allow(dead_code)]
...
mod tests {
    ...
    /// Includes zero, one, negatives, and non-trivial denominators. Tests tagged
    /// `[PROP]` state an algebraic law and discharge it by exhaustive enumeration
    /// over this finite domain (no property-testing dependency needed).
