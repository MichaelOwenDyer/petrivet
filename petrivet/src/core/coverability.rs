...
pub fn find_candidate_covering_parikh_vector(
    ...
    let extract_u32 = |model: &Model, tm: &TermManager, term_id: TermId| {
        let evaluated_term = model.get(term_id).tap_none(|| {
            eprintln!("warning! model did not contain {term_id:?}. model: {model:?}")
        })?;
    ...
    loop {
        ...
        {
            let candidate_solution: IdxMarking<u32> = place_terms
                .iter()
                .copied()
            ...
            match &transition_terms {
                ...
                _ if let Some(invariant) =
                    find_violated_s_invariant(&incidence_matrix, m0, &candidate_solution) =>
                {
                    let constant_tokens: u32 = invariant.iter().map(|&(p, w)| w * m0[p]).sum();
                    let weighted_places: Vec<_> = invariant
                        .iter()
                        .map(|&(p, w)| {
                ...
                _ if let Some(trap) =
                    find_violated_initially_marked_trap(net, m0, &candidate_solution) =>
                {
                ...
                Some(transition_terms) => {
                    let parikh: Vec<u32> = transition_terms
                        .iter()
                        .copied()
            ...
            if result != SolverResult::Unsat {
                eprintln!(
                    "warning! unexpected oxiz solver state!\nDebug info:\n{solver:#?}\n{tm:#?}"
                );
            }
            return None;
        }
...
fn find_violated_s_invariant(
    ...
    let weights: Vec<_> = incidence_matrix
        .place_indices()
        .map(|p_idx| tm.mk_var(&format!("p{p_idx}"), tm.sorts.int_sort))
        .collect();
    ...
    for t_idx in incidence_matrix.transition_indices() {
        let muls: Vec<_> = incidence_matrix
            .place_indices()
        ...
        && let Some(model) = solver.model()
    {
        let invariant: Vec<(PlaceIdx, u32)> = incidence_matrix
            ...
            .filter_map(|(p_idx, weight_term)| {
                let val_id = model.get(weight_term).tap_none(|| {
                    eprintln!("warning! no model assignment for weight {weight_term:?}")
                })?;
                if let TermKind::IntConst(n) = &tm.get(val_id)?.kind {
                    let w = u32::try_from(n).ok().tap_none(|| {
                        eprintln!("warning! weight {val_id:?} is not a valid u32: {n}")
                    })?;
                    (w > 0).then_some((p_idx, w))
                } else {
                    eprintln!(
                        "warning! weight {weight_term:?} has unexpected kind: {:?}",
                        tm.get(val_id).map(|t| &t.kind)
                    );
                    None
                }
            })
...
fn find_violated_initially_marked_trap(
    ...
    let mut worklist: Vec<PlaceIdx> = net.place_indices().filter(|&p| in_trap[p]).collect();
