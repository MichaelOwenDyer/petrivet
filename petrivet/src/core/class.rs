...
pub enum NetClass {
    ...
    /// A marking M is reachable from M₀ iff there exists X ∈ ℕ<sup>|T|</sup> such that:
    /// - M = M₀ + N * X, where N is the incidence matrix of N
    /// - (N<sub>U</sub>, M<sub>U</sub>) has no unmarked traps,
    ///   where U = supp(X) = {t ∈ T | X(t) > 0} is the set of *fired* transitions,
    ///   N<sub>U</sub> is the subnet generated by U, and M<sub>U</sub> is the
    ///   projection of the *target* M onto the places of N<sub>U</sub>.
    ///
    /// (The trap condition is on the fired transitions and the target marking.
    /// This mirrors the T-net special case — Theorem 22, where the support of X
    /// induces the subnet — and was verified exhaustively against BFS
    /// reachability; see `PetriNet::analyze_reachability` and its `fc_*` tests.)
    ///
