...
impl<N: AsRef<Net>> PetriNet<N> {
    ...
    pub fn commoner_hack_criterion(&self) -> CommonerHackCriterionResult {
        let siphon_trap_pairs = siphon_trap::commoner_hack_criterion(
            ...
            let siphon = siphon.into_iter().map(|p_idx| self.core.net.as_ref().mapping.place_key(p_idx)).collect();
            let trap = trap.into_iter().map(|p_idx| self.core.net.as_ref().mapping.place_key(p_idx)).collect();
    ...
    pub fn analyze_reachability(&self, target: Marking) -> ReachabilityResult {
        ...
        match self.build_coverability_graph().into_reachability_graph() {
            Ok(rg) => {
                rg.path_to_marking(&target).map_or_else(
    ...
    fn coverability_unbounded_omega_witness() {
        ...
        match res {
            CoverabilityResult::Coverable(CoverabilityProof { covering_marking, .. }) => {
                ...
                assert_eq!(covering_marking.get(p0).unwrap(), &Omega::Finite(1));
                assert!(covering_marking.get(p1).unwrap() >= &Omega::Finite(100_000));
            }
