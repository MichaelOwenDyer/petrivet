...
//! Used in: [`System::analyze_reachability`](crate::PetriNet::analyze_reachability).
//!
//! ## Theorem 5.3.18 — Reachability in live bounded free-choice systems
//!
//! > Let (N, M₀) be a live and bounded free-choice system and let M be a
//! > marking. M is reachable from M₀ iff there is a non-negative integer
//! > solution X of the marking equation M = M₀ + N·X such that the subnet
//! > generated by the support of X (the fired transitions) contains no trap
//! > that is unmarked under M.
//!
//! This is the free-choice generalisation of the T-net theorem above: the
//! support of X induces the subnet, and a token-free trap plays the role of the
//! token-free circuit. All three hypotheses are decided in polynomial time —
//! free-choice is structural, liveness is the Commoner/Hack criterion, and
//! boundedness is witnessed by a positive place sub-invariant — so reachability
//! is decidable in polynomial time.
//!
//! The decider tests the trap condition on the minimum-firing solution. A
//! passing solution proves reachability (the theorem's sufficiency direction);
//! an infeasible marking equation proves unreachability. When the equation is
//! feasible but the trap check fails, the procedure abstains (returns
//! `Inconclusive`) rather than assert a verdict, because a different solution
//! might still witness reachability. The reading (fired transitions, target
//! marking) was verified exhaustively against BFS reachability.
//!
//! References: Desel & Esparza, *Free Choice Petri Nets*.
//!
//! Used in: [`System::analyze_reachability`](crate::PetriNet::analyze_reachability).
//!
