...
impl<'a> CoverabilityExplorer<'a> {
    ...
    pub fn explore_next(&mut self) -> Option<CoverabilityStep> {
        loop {
            ...
            let transition_key = self.explorer.state_space.net.get_transition(dense_t);
...
impl<'a> CoverabilityGraph<'a> {
    ...
    pub fn place_bound(&self, p: Place) -> Omega {
        self.state_space.net
            .place_index(p)
            .map(|idx| self.place_bound_dense(idx))
            .unwrap_or_else(|| Omega::Finite(0))
    }

    /// Dense-index version for internal use.
    fn place_bound_dense(&self, p: PlaceIdx) -> Omega {
        self.state_space
            .graph
            .node_weights()
            .map(|m| m[p])
